...
S02: Based on the `.rdd-docs/workspace/implementation-plan.md` update `.rdd-docs/workspace/copilot-prompts.md` by creating a series of prompts which should be executed sequentially and which should define in very details the needed outcome. In prompts refer to `.rdd-docs/workspace/implementation-plan.md`, if needed, mentioning the exact place in the implementation plan file to be found the information. Also refer to other files in .rdd-docs/ folder and subfolders if needed. Follow the format in the `.rdd-docs/workspace/copilot-prompts.md`. You can reuse the promts where <PROMPT-PLACEHOLDER> is the only entry and you can create new prompts continuing the sequence. **Do not change existing prompts**.
