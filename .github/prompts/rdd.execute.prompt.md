...
- [PROMPT-NAME] is the value of the attribute "PROMPT-NAME" in the file `.rdd-instance/workdir/work-iteration-registry.json`
...
- Find the values of the parameters "PROMPT-ID" and "PROMPT-NAME" from `.rdd-instance\workspace\work-iteration-registry.json` registry file.  
...
- Never change the `.rdd-instance\workspace\work-iteration-prompt.md` file! If the file is missing or malformed - stop and inform the user.  
...
- Read again the content of [WI-REGISTRY] (as it could be manually changed by the user meanwhile) before eachq step to decide if the step should be executed or skipped.
