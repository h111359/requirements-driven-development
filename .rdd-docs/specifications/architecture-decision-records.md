...
[ADR-001] An empty workspace folder `.rdd-docs/workspace` should exist at the start of each new work iteration
...
[ADR-011] The file `.rdd-docs/work-iteration-prompt.md` is the single active prompt used for execution.
...
[ADR-014] The configuration file for the framework is stored at `.rdd-docs/config.json`.
...
[ADR-020] Archived workspaces are stored in `.rdd-docs/archive/`.
