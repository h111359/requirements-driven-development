...
[ADDED] The RDD framework provides a structured workflow methodology for software development integrating requirements clarification, change tracking, and documentation management, optimized for GitHub Copilot usage.
...
- **[MODIFIED] [GF-11] Interactive Command-Line Interface**: The framework shall provide an interactive menu system when rdd.sh is executed without parameters, allowing users to select from numbered menu options covering all major RDD operations.
- **[ADDED] Prompt-Based Workflow Execution**: The framework shall provide a series of prompt files that guide the user through each phase of the RDD workflow, from change initiation to wrap-up.
...
- **[ADDED] Interactive Change Name Normalization**: When creating a new change, the system shall prompt for a name, automatically normalize it to kebab-case, display the normalized result, and request user confirmation before proceeding.
- **[ADDED] Change Description Capture**: The change creation workflow shall prompt for a short description and store it for reference and logging purposes.
- **[ADDED] Modular Script Architecture**: The rdd.sh script shall source utility scripts from separate domain-specific modules (core-utils, git-utils, branch-utils, workspace-utils, change-utils, etc.) to maintain code organization and reusability.
- **[ADDED] Domain-Based Routing**: The rdd.sh script shall route commands based on domain names (branch, workspace, requirements, change, fix, clarify, prompt, pr, git) with corresponding actions for each domain.
- **[ADDED] Copilot Prompts File Management**: The system shall maintain a copilot-prompts.md file in workspace containing a list of stand-alone prompts that can be executed sequentially, with checkboxes to track completion status.
- **[ADDED] Stand-Alone Prompt Execution**: The framework shall provide a mechanism to execute stand-alone prompts from copilot-prompts.md by prompt ID, marking them as completed after execution and logging details to log.jsonl.
...
- **[ADDED] User-Friendly Naming**: The system shall accept change names in any format (with spaces, mixed case, special characters) and automatically normalize them to the required kebab-case format without user intervention.
- **[ADDED] Interactive Menu Usability**: The interactive menu shall display all options with clear numbering, allow selection by entering a number, and automatically return to the menu after each operation for continued use.
...
- **[ADDED] Utility Script Modularity**: Utility scripts (core-utils.sh, git-utils.sh, branch-utils.sh, workspace-utils.sh, clarify-utils.sh, requirements-utils.sh, prompt-utils.sh, pr-utils.sh, change-utils.sh, fix-management.sh, general.sh) shall be organized by functional domain and independently sourceable.
- **[ADDED] Exported Functions**: All utility scripts shall export their functions using export -f to make them available to scripts that source them.
- **[ADDED] Multiple Sourcing Protection**: Utility scripts shall prevent multiple sourcing by checking for a _LOADED variable and returning early if already sourced.
- **[ADDED] Name Normalization Function**: The core-utils.sh script shall provide a normalize_to_kebab_case function that converts any input string to lowercase kebab-case format by replacing spaces and special characters with hyphens and removing consecutive hyphens.
- **[ADDED] Interactive Menu System**: The rdd.sh script shall implement an interactive menu using bash select statement that displays numbered options and accepts numeric input from the user.
- **[ADDED] JSONL Logging Format**: Execution logs shall be stored in JSONL (JSON Lines) format with one JSON object per line, containing timestamp, promptId, executionDetails, and sessionId fields.
- **[ADDED] Prompt File Template**: The copilot-prompts.md template shall use markdown checkboxes (- [ ]) for unchecked prompts and (- [x]) for completed prompts, with prompt IDs in square brackets (e.g., [P01]).
