...
- **Archive Creation**: Generates single `rdd-v{version}.zip` file containing:
  ...
  - Seed templates (.rdd-instance/ with config.json, data-model.md, requirements.md)
...
5. Create build directory structure (including .rdd-instance/)
...
8. Copy seed templates to .rdd-instance/ (config.json, data-model.md, requirements.md)
...
- **Installation Description**: Displays clear preview of actions before prompting for directory:
  ...
  - Copy seed templates (.rdd-instance/)
...
- **Enhanced existing installation detection**:
  - Scans for .rdd/, .github/prompts/, .rdd-instance/ directories
...
- **Obsolete file archiving** (upgrade scenarios):
  ...
  - Creates archive directory `.rdd-instance/archive/installation_<version>/`
