...
- [FR-002] The system shall support defining multiple User Stories in `.rdd-instance/workdir/rdd-prompt-setup.json`, where each User Story has its own `prompt-file` and its own stage tracking (`context`, `clarity`, `plan`, `implementation`), and where `tasks` remain a separate, top-level queue that can be executed independently from User Stories.
...
- [FR-069] The system shall treat the `prompt-file` referenced by the active User Story in `.rdd-instance/workdir/rdd-prompt-setup.json` as the single authoritative and complete definition of the work to be executed, and the `execute command` shall consume the selected `prompt-file` content verbatim when performing an execution in `userStory` mode.
...
- [TR-005] The framework shall implement User Story prompts as Markdown files referenced by `userStories[].prompt-file` in `.rdd-instance/workdir/rdd-prompt-setup.json`, and the selected User Story prompt contents shall be consumed entirely by the `execute` command when running in `userStory` mode.
...
- [TR-026] The Web UI shall load and save User Story prompt files referenced by `.rdd-instance/workdir/rdd-prompt-setup.json` (`userStories[].prompt-file`), persist questionnaire responses for the active User Story, and present implementation plans generated by the `execute command`.
