...
2.  Update [REQUIREMENTS] following the instructions in `.rdd/conventions/requirements.convention.md` so to reflect precisely the changes from the prompt (if not reflected already). If reflected - do not duplicate.
...
**IMPORTANT**: Never edit [REQUIREMENTS] directly. Always use requirement scripts.
