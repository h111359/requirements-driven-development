#!/usr/bin/env python3
"""Create a new User Requirement in requirements.md.

This script enforces safe, deterministic requirement creation with automatic
ID generation and format validation.

Usage:
  requirement_ur_create.py text="<requirement text>" [validation=basic|none]
...
Output:
  Prints "SUCCESS: Created UR-XXXX" to stdout on success
  Prints error message to stderr on failure
  Exit code: 0 for success, 1 for failure

Examples:
  python requirement_ur_create.py text="The system shall export data in CSV format"
  python requirement_ur_create.py text="See external spec" validation=none
"""
...
_REQUIREMENT_ID_RE = re.compile(r"\[UR-(\d{4})\]")
_VALIDATION_MODES = {"basic", "none"}


def _repo_root() -> Path:
    """Get repository root directory."""
    # This file lives at: <repo>/.rdd/src/actions/requirement_ur_create.py
...
def _find_highest_ur_id(content: str) -> int:
    """Find the highest existing UR ID number in requirements.md.
...
def _find_section_end(content: str, section_header: str) -> Optional[int]:
    ...
    Args:
        content: Content of requirements.md file
        section_header: Section header to find (e.g., "## User Requirements")
...
def _create_requirement(
    ...
    Returns:
        Created requirement ID (e.g., "UR-0042")
    ...
    # Find highest existing UR ID
    highest_id = _find_highest_ur_id(content)
    new_id_num = highest_id + 1
    new_id = f"UR-{new_id_num:04d}"
    
    # Find where to insert the new requirement
    section_end = _find_section_end(content, "## User Requirements")
    if section_end is None:
        raise ValueError(
            "'## User Requirements' section not found in requirements.md. "
...
def main() -> int:
    ...
    if not text or not text.strip():
        print(
            ...
            "Usage: requirement_ur_create.py text=\"The system shall...\"",
            file=sys.stderr
        )
        return 1
