#!/usr/bin/env python3
"""Modify an existing Technical Requirement in requirements.md.

This script safely modifies requirement text while preserving ID and file format.

Usage:
  requirement_tr_modify.py id="TR-XXXX" text="<new requirement text>" [validation=basic|none]

Parameters:
  id - The requirement ID to modify (required, format: TR-XXXX)
  text - The new requirement text (required, 10-2048 characters)
  validation - Validation level: 'basic' (default) or 'none'

Output:
  Prints "SUCCESS: Modified TR-XXXX" to stdout on success
  Prints error message to stderr on failure
  Exit code: 0 for success, 1 for failure

Examples:
  python requirement_tr_modify.py id="TR-0042" text="The framework shall use Python 3.11 or higher"
  python requirement_tr_modify.py id="TR-0023" text="See spec v2" validation=none
"""
...
def _repo_root() -> Path:
    ...
    # This file lives at: <repo>/.rdd/src/actions/requirement_tr_modify.py
...
def _modify_requirement(
    ...
    """Modify an existing Technical Requirement.
    ...
    if not match:
        raise ValueError(
            ...
            f"Use requirement_tr_create.py to create new requirements."
        )
...
def main() -> int:
    ...
    if not requirement_id or not requirement_id.strip():
        print(
            ...
            "Usage: requirement_tr_modify.py id=\"TR-XXXX\" text=\"New text...\"",
            file=sys.stderr
        )
        return 1
