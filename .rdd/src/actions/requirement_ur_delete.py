#!/usr/bin/env python3
"""Delete (mark as [DELETED]) a User Requirement in requirements.md.
...
Usage:
  requirement_ur_delete.py id="UR-XXXX"

Parameters:
  id - The requirement ID to delete (required, format: UR-XXXX)

Output:
  Prints "SUCCESS: Deleted UR-XXXX" to stdout on success
  Prints error message to stderr on failure
  Exit code: 0 for success, 1 for failure

Examples:
  python requirement_ur_delete.py id="UR-0088"
"""
...
def _repo_root() -> Path:
    ...
    # This file lives at: <repo>/.rdd/src/actions/requirement_ur_delete.py
...
def _delete_requirement(
    ...
    """Delete (mark as [DELETED]) a User Requirement.
...
def main() -> int:
    ...
    if not requirement_id or not requirement_id.strip():
        print(
            ...
            "Usage: requirement_ur_delete.py id=\"UR-XXXX\"",
            file=sys.stderr
        )
        return 1
