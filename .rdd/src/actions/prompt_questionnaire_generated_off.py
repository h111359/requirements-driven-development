#!/usr/bin/env python3
"""
Set questionnaire-generated flag to false for a prompt.

This script sets the questionnaire-generated flag to false for a specified prompt
in the work iteration registry. If no prompt-id is provided, it defaults
to the currently active prompt (one in 'active' state).

Usage:
    python prompt_questionnaire_generated_off.py [prompt-id=<id>]

Example:
    python prompt_questionnaire_generated_off.py prompt-id=P-003
    python prompt_questionnaire_generated_off.py  # Uses active prompt
"""
...
def main():
    ...
    exit_code = set_questionnaire_generated(prompt_id, False)
    sys.exit(exit_code)
