...
def enable_plan_mode(prompt_id=None):
    ...
    if target_prompt.get('state') != 'active':
        print(f"ERROR: Cannot enable plan mode for prompt '{prompt_id}' in state '{target_prompt.get('state')}'")
        print("REMEDIATION: Plan mode can only be enabled for prompts in 'active' state.")
        return 1
