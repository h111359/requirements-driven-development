...
This script sets the analyze-enabled flag to false for a specified prompt
...
Usage:
    python prompt_analyze_off.py [prompt-id=<id>]

Example:
    python prompt_analyze_off.py prompt-id=P-003
    python prompt_analyze_off.py  # Uses active prompt
"""
...
def disable_analyze_mode(prompt_id=None):
    """
    Disable analyze mode for the specified prompt.
    
    Args:
        prompt_id (str, optional): Prompt ID to disable analyze for.
    ...
    print(f"SUCCESS: Analyze mode disabled for prompt '{prompt_id}'")
    return 0
...
def main():
    ...
    exit_code = disable_analyze_mode(prompt_id)
    sys.exit(exit_code)
