...
- Copies documentation templates to `.rdd-instance/`
...
> **Note**: Configuration changes are saved to `.rdd-instance/config.json` and persist across sessions.
...
2. DEFINE WORK IN PROMPTS FILE
   • Edit .rdd-instance/work-iteration-prompts.md
...
- ✓ Workspace must be empty (`.rdd-instance/workdir/` should not exist or be empty)
...
4. **Workspace Initialization**: The workspace is initialized with a prompts file
   - File created: `.rdd-instance/work-iteration-prompts.md`
...
The heart of RDD development is the `work-iteration-prompts.md` file where you define specific tasks for GitHub Copilot to execute. It is not mandatory to have fulfilled `.rdd-instance/user-story.md` file in order to work with `.rdd-instance/work-iteration-prompts.md`. For fixes the intended approach will be to directly write your prompts in `.rdd-instance/work-iteration-prompts.md` and run the /rdd.execute command.

### The Prompts File Structure

**Location**: `.rdd-instance/work-iteration-prompts.md`
...
Open `.rdd-instance/work-iteration-prompts.md` and add a prompt for Copilot to execute. When you have finished with formulation of the prompt text, save the file and proceed with the chat:
...
1. **Reads the prompt** from `.rdd-instance/work-iteration-prompts.md`
2. **Reads context files** (`.rdd-instance/requirements.md`, `.rdd-instance/tech-spec.md`)
3. **Executes the instructions** (creates files, writes code, runs tests)
4. **Creates implementation file** at `.rdd-instance/workdir/P01-implementation.md`
...
**File**: `.rdd-instance/workdir/P<ID>-implementation.md`
...
**What It Does**:
1. Analyzes all implementation files in `.rdd-instance/workdir/`
2. Identifies changes (new features, bug fixes, documentation updates)
3. Updates `.rdd-instance/requirements.md` with new/modified requirements
4. Updates `.rdd-instance/tech-spec.md` (including Project Folder Structure and Data Architecture sections)
...
- Better way is to open `.rdd-instance/user-story.md` file and to fulfill the sections:
  - What is needed?
...
- When the questionnaire is generated, open again `.rdd-instance/user-story.md` and fulfill your answers to the questions and after that issue again `/rdd.analyse-and-plan` command
...
- Revise the prompts and use them manully to fulfill the `.rdd-instance/work-iteration-prompts.md` with them (or their modification if you prefer to make some tweeks)
...
**What It Does**:
1. Reads your user story from `.rdd-instance/user-story.md`
2. Guides you through requirement clarification using a state-based workflow
3. Generates a Requirements Questionnaire with multiple-choice questions in the `.rdd-instance/user-story.md` itself
4. Creates a detailed execution plan with sequenced prompts in the `.rdd-instance/user-story.md` itself
...
When all prompts in `.rdd-instance/work-iteration-prompts.md` are executed and documentation is updated, complete the iteration using menu option 3.
...
1. **Workspace Archiving**:
   - All files from `.rdd-instance/workdir/` are copied to `.rdd-instance/archive/<branch-name>/`
   - Archive metadata file created with timestamp, branch name, author, commit info
   - Example archive location: `.rdd-instance/archive/fix-user-login-bug/`
...
- All work results contained in `.rdd-instance/` directory
...
- Archives in `.rdd-instance/archive/<branch-name>/`
...
```markdown
 - [ ] [P01] Add user registration endpoint following the patterns in 
   .rdd-instance/tech-spec.md section "API Design Patterns". Create endpoint 
...
- `.rdd-instance/requirements.md` for functional requirements
- `.rdd-instance/tech-spec.md` for architecture and patterns
...
- Documentation: Check `.rdd-instance/` in your project
