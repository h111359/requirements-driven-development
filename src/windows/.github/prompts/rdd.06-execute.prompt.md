...
4. **Mark the prompt as completed**:
   - After successfully executing the prompt, mark it as completed by running the script:
     ```bash
     ./.rdd/scripts/rdd.ps1 prompt mark-completed <PROMPT_ID>
     ```
...
5. **Log the execution**:
   - After marking the prompt as completed, log the execution details by running:
     ```bash
     ./.rdd/scripts/rdd.ps1 prompt log-execution <PROMPT_ID> "<EXECUTION_DETAILS>"
     ```
   - Replace `<PROMPT_ID>` with the actual prompt ID (e.g., `P001`, `P002`, etc.)
...
- **Use the script for marking**: Always use `./.rdd/scripts/rdd.ps1 prompt mark-completed <PROMPT_ID>` to mark prompts as completed. Never manually edit the .rdd.copilot-prompts.md file.
- **Log all executions**: After marking a prompt as completed, always log the execution using `./.rdd/scripts/rdd.ps1 prompt log-execution <PROMPT_ID> "<EXECUTION_DETAILS>"`.
...
6. You mark P001 as completed by running: `./.rdd/scripts/rdd.ps1 prompt mark-completed P001`
7. You log the execution by running: `./.rdd/scripts/rdd.ps1 prompt log-execution P001 "Created prompt file with instructions for executing stand-alone prompts"`
